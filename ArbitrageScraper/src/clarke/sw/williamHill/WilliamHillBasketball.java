...
public class WilliamHillBasketball extends MakeMatches implements RetrieveOdds, Callable<LinkedList<Matches>> {
	...
	private String page = "http://sports.williamhill.com/bet/en-gb/betting/y/3/cp/496/Basketball.html";
	...
	public LinkedList<Player> getOdds(Document doc) {
		...
		for (Element e : doc.select("table")) {
			if (e.select("thead").select("tr").select("th").select("span").text().contains("Enhanced")) {
				e.remove();
			}
			if (e.select("thead").select("tr").select("th").select("span").text().contains("NBA")) {
				e.remove();
			}
			if (e.select("thead").select("tr").select("th").select("span").text().contains("Handicap")) {
				e.remove();
			}
			if (e.select("thead").select("tr").select("th").select("span").text().contains("Spread")) {
				e.remove();
			}
		}
		...
		playersOdds = doc.select("table[class=tableData]").select("tbody").select("tr[class=rowOdd]")
				.select("div[class=eventpriceholder-left").select(".eventprice");
	...
	public LinkedList<Matches> call() throws Exception {
	 ...
	 WilliamHillBasketball wh = new WilliamHillBasketball();
	 Document doc =
	 wh.getConnection("http://sports.williamhill.com/bet/en-gb/betting/y/3/cp/496/Basketball.html");
	 LinkedList<Player> players = wh.getOdds(doc);
	 LinkedList<Matches> matches = wh.getMatches(players, Sports.BASKETBALL);
